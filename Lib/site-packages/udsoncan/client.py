...
class Client:
    ...
    Object that interacts with a UDS1 server.
...
# ====  ReadDTCInformation
    ...
    def get_dtc_fault_counter(self) -> Optional[services.ReadDTCInformation.InterpretedResponse]:
        ...
        If the ECU follows the UDS1 guidelines, it will wait to detect a fault many times before setting a status bit for this fault DTC. Each time the fault is detected, a fault counter is incremented, when it is not detected, the counter is decremented.
            ...
            if not response.positive:
                try:
                    if not Response.Code.is_supported_by_standard(response.code, self.config['standard_version']):
                        self.logger.warning('Given response code "%s" (0x%02x) is not supported byt the UDS1 standard version that the clients s enforcing (%s)' % (
                            response.code_name,
                            response.code,
                ...
                if not request.service.is_supported_negative_response(response.code):
                    self.logger.warning('Given response code "%s" (0x%02x) is not a supported negative response code according to UDS1 standard.' % (
