            ...
            for index, channel in enumerate(self.channels):
                ...
                if _os.lower() == 'windows':
                    ...
                    if bitrate is None:
                        ...
                        init_config['baud_rate'] = bitrate
                        del config['bitrate']

                    data_bitrate = config.get('data_bitrate', None)
                    if data_bitrate is not None:
                        init_config['canfd_dbit_baud_rate'] = data_bitrate
                    ...
                    if hasattr(self.device, 'SetValue'):
                        ...
                        #     pass
                        self.device.SetValue(channel, **init_config)
                elif _os.lower() == 'linux':
                    init_config = config
    ...
    def poll_received_messages(self, timeout):
        try:
            for channel in self.available:
                ...
                if can_num:
                    print(f'ZLG-CAN - can message received: {can_num}.')
