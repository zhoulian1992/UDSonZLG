...
class _ZCANWindows(_ZLGCAN):
    ...
    def _get_can_init_config(self, mode, filter, **kwargs):
        ...
        configinfo = ZCAN_CHANNEL_INIT_CONFIG()
        assert self._dev_is_canfd is not None, f'The device{self._dev_index} is not opened!'
        # clock = kwargs.get('clock', None)
        # if clock:
        #     self.SetValue()
        configinfo.can_type = ZCANCanType.CANFD if self._dev_is_canfd else ZCANCanType.CAN
        ...
        if self._dev_is_canfd:
            ...
            if self._dev_type not in ZUSBCANFD_TYPE:
                configinfo.config.canfd.acc_code = acc_code
                configinfo.config.canfd.acc_mask = acc_mask
            configinfo.config.canfd.abit_timing = kwargs.get('abit_timing', 104286)     # ignored
            configinfo.config.canfd.dbit_timing = kwargs.get('dbit_timing', 8487694)    # ignored
            configinfo.config.canfd.brp = kwargs.get('brp', 0)
            configinfo.config.canfd.filter = filter
            configinfo.config.canfd.mode = mode
            configinfo.config.canfd.brp = kwargs.get('pad', 0)
        else:
            if self._dev_type in (ZCANDeviceType.ZCAN_PCI5010U, ZCANDeviceType.ZCAN_PCI5020U,
                ...
                configinfo.config.can.acc_code = acc_code
                configinfo.config.can.acc_mask = acc_mask
            configinfo.config.can.filter = filter
            if self._dev_type in ZUSBCAN_I_II_TYPE:
                # configinfo.config.can.timing0 = kwargs.get('timing0', 0)                   # ignored
                # configinfo.config.can.timing1 = kwargs.get('timing0', 28)                  # ignored
                configinfo.config.can.acc_code = acc_code
                configinfo.config.can.acc_mask = acc_mask
            configinfo.config.can.mode = mode
        return configinfo
